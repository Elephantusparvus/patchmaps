...
def get_structure(poly: Polygon, crs='epsg:4326', working_width=36, factor=2, tramline=None) -> gpd.GeoDataFrame:
    ...
    angle = angle_between(np.array([x_diff, y_diff]), direction)

    rotated = shapely.affinity.rotate(poly, angle, origin=(centroid.x, centroid.y), use_radians=True)
    ...
    def compute_poly(i, j):
        ...
        return shapely.affinity.rotate(patch, -angle, origin=(centroid.x, centroid.y), use_radians=True)
